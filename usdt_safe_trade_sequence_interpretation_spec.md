...
| 컴포넌트 | 설명 |
|---|---|
| Mermaid 시퀀스 뷰어 | 최종 개선 시퀀스를 단일 기준으로 표시하며 이전 시퀀스는 백업 원문으로만 보관 |
| STEP 타임라인 | 사전준비, 매칭준비중, 매칭확정, STEP1-1, STEP1-2, STEP2, 완료 기준으로 표시 |
...
본 화면은 최종 개선 시퀀스를 단일 기준으로 해석한다. 기존 STEP1~STEP5 방식은 백업 원문으로만 보관하고 화면에는 노출하지 않는다.
...
| STEP1-1 | 지갑주소 확인과 USDT 전송 | USDT 입금 자동검증 대기 | 매수자 수령지갑과 포블 수수료 지갑으로 USDT 전송 | 수령지갑/수수료 지갑 입금 검증 완료 |
| STEP1-2 | USDT 확인과 원화 입금 | USDT 확인, 원화계좌 확인, 이체확인증 등록, 입금완료 메시지 | 입금완료 메시지 대기 | 이체확인증 등록과 입금완료 메시지 송출 |
...
STEP 번호는 역할명이 아니라 진행 순서를 뜻한다. 역할별 책임은 각 STEP 상세 카드 안에서 매수자/매도자/시스템/운영 기준으로 분리해 보여준다.

---

## 6. 전체 Mermaid 시퀀스

웹 뷰에서는 전체 시퀀스 영역을 최종 개선 시퀀스 단일 기준으로 제공한다. 이전 시퀀스는 비교 탭으로 노출하지 않고 백업 원문으로만 보관한다. 최종 시퀀스에서는 매도자가 신청 전에 거래금액 선택과 1원 인증을 완료하고, 매수자가 신청 전에 수령지갑 생성 약관에 동의한다. 실제 지갑주소 생성은 매칭확정과 PIN 입력 후 수행하며, USDT 자동검증이 완료되면 매수자에게 원화 계좌를 즉시 노출하고 매도자는 원화 수취 확인 화면으로 진행한다.

고객 화면은 기존 공통 STEP1~STEP5가 아니라 `사전준비 → 매칭준비중 → 매칭확정 → STEP1-1 → STEP1-2 → STEP2 → 완료`로 해석한다. 양측 신청이 매칭된 직후에는 `매칭준비중` 상태로 진입하며, 내 거래 의사 메시지 보내기, 약관 확인, PIN 입력을 완료해야 한다. 양측 PIN 입력이 완료되기 전까지는 매칭취소가 가능하고, 양측 PIN 입력과 필요 시 ARS 동의가 완료되어 `매칭확정`으로 전환된 이후에는 임의 취소가 불가하다.

| 역할 | STEP | 화면 의미 | 핵심 조건 |
|---|---|---|---|
| 매도자 | STEP1-1 | 매수자 수령지갑과 포블 수수료 지갑 주소를 확인하고 USDT를 전송하는 화면 | 매칭확정, 지갑주소 생성 |
| 매수자 | STEP1-2 | 수령지갑 USDT 입금 자동검증 후 원화 계좌를 확인하고 이체확인증을 등록하는 화면 | USDT 및 수수료 자동검증 완료 |
