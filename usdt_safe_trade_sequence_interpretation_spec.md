...
개선 시퀀스의 고객 화면은 공통 STEP1~STEP5보다 역할별 2 STEP 구조로 해석한다.

| 역할 | STEP | 화면 의미 | 핵심 조건 |
|---|---|---|---|
| 매도자 | STEP1 | 매수자 수령지갑과 포블 수수료 지갑 주소를 확인하고 USDT를 전송하는 화면 | 매칭확정, PIN 완료, 지갑주소 생성 |
| 매도자 | STEP2 | 매수자 입금완료 메시지 수신 후 원화 입금을 확인하고 완료 대기하는 화면 | 매수자 이체확인증 등록, 입금완료 메시지 |
| 매수자 | STEP1 | 수령지갑 USDT 입금 자동검증 후 원화 계좌를 확인하고 이체확인증을 등록하는 화면 | USDT 및 수수료 자동검증 완료 |
...
2. 이체확인증 등록과 본인 이체 확인 동의 전에는 매도자에게 입금완료 메시지를 송출하지 않는다.
3. 매도자 원화 입금확인 완료와 거래소 이체확인증 검토 완료가 모두 충족되기 전에는 니모닉 조회 버튼을 활성화하지 않는다.
4. 매수자가 니모닉을 조회하면 거래완료로 전환하고 접근 로그를 저장한다.
