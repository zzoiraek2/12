...
| 컴포넌트 | 설명 |
|---|---|
| Mermaid 시퀀스 뷰어 | 기존 시퀀스와 개선 시퀀스를 탭으로 비교 표시 |
| STEP 타임라인 | 선택한 시퀀스 탭에 맞춰 기존 STEP1~STEP5 또는 개선안 역할별 단계로 전환 표시 |
...
기존 시퀀스 탭에서는 기존 정책 검토를 위해 STEP1~STEP5 구조를 유지한다.
...
개선 시퀀스 탭에서는 공통 STEP1~STEP5를 그대로 노출하지 않고, 신청 전 준비와 역할별 진행 화면을 기준으로 STEP 해석을 전환한다.

| 단계 | 화면 의미 | 매수자 핵심 행동 | 매도자 핵심 행동 | 완료 조건 |
|---|---|---|---|---|
| 사전준비 | 신청 전 이용조건 준비 | 수령지갑 생성 및 조건부 해제 절차 동의 | 거래금액 선택, 본인명의 원화 수취계좌 1원 인증 | 신청 가능 조건 충족 |
| 매칭준비중 | 의사 확인과 PIN 입력 | 내 거래 의사 메시지, 약관 동의, PIN 입력 | 내 거래 의사 메시지, 약관 동의, PIN 입력 | 양측 메시지와 PIN 완료 |
| 매칭확정 | 취소불가 전환과 지갑주소 생성 | 매칭확정 및 취소불가 안내 확인 | 매칭확정 및 취소불가 안내 확인 | MATCH_CONFIRMED, 지갑주소 생성 |
| 매도자 STEP1 | 지갑주소 확인과 USDT 전송 | USDT 입금 자동검증 대기 | 매수자 수령지갑과 포블 수수료 지갑으로 USDT 전송 | 수령지갑/수수료 지갑 입금 검증 완료 |
| 매수자 STEP1 | USDT 확인과 원화 입금 | USDT 확인, 원화계좌 확인, 이체확인증 등록, 입금완료 메시지 | 입금완료 메시지 대기 | 이체확인증 등록과 입금완료 메시지 송출 |
| STEP2 | 원화 확인과 니모닉 열람 | 매도자 확인 및 거래소 검토 후 니모닉 조회 | 원화 입금 확인 완료 버튼 클릭 | 니모닉 조회 시 거래완료 |
| 완료 | 거래완료와 기록 보관 | 니모닉 별도 보관 및 자산 소유 확인 | 거래 종료 확인 | 로그/증빙 보관 |

이 화면 전환 기준은 전체 시퀀스 Mermaid 탭과 동기화한다. 사용자가 `개선 시퀀스`를 선택하면 STEP 해석, 상세 카드, 다운로드 요약도 개선안 단계 기준으로 표시한다.

---

## 6. 전체 Mermaid 시퀀스

웹 뷰에서는 전체 시퀀스 영역을 `기존 시퀀스`와 `개선 시퀀스` 탭으로 제공한다. 기존 시퀀스는 정책 검토용 기본 흐름이며, 개선 시퀀스는 신청 전 준비를 앞당겨 매칭 후 대기시간을 줄이는 UX 흐름이다. 개선 시퀀스에서는 매도자가 신청 전에 거래금액 선택과 1원 인증을 완료하고, 매수자가 신청 전에 수령지갑 생성 약관에 동의한다. 실제 지갑주소 생성은 매칭확정과 PIN 입력 후 수행하며, USDT 자동검증이 완료되면 매수자에게 원화 계좌를 즉시 노출하고 매도자는 원화 수취 확인 화면으로 진행한다.

개선 시퀀스의 고객 화면은 공통 STEP1~STEP5보다 역할별 2 STEP 구조로 해석한다. 단, 양측 신청이 매칭된 직후에는 기존 정책과 동일하게 `매칭준비중` 상태로 진입하며, 각자의 STEP1 앞부분에서 내 거래 의사 메시지 보내기, 약관 확인, PIN 입력을 완료해야 한다. 양측 PIN 입력이 완료되기 전까지는 매칭취소가 가능하고, 양측 PIN 입력과 필요 시 ARS 동의가 완료되어 `매칭확정`으로 전환된 이후에는 임의 취소가 불가하다.

| 역할 | STEP | 화면 의미 | 핵심 조건 |
|---|---|---|---|
| 매도자 | STEP1 | 거래 의사 메시지/PIN을 완료한 뒤 매수자 수령지갑과 포블 수수료 지갑 주소를 확인하고 USDT를 전송하는 화면 | 매칭준비중, 양측 PIN 완료, 매칭확정, 지갑주소 생성 |
| 매도자 | STEP2 | 매수자 입금완료 메시지 수신 후 원화 입금을 확인하고 완료 대기하는 화면 | 매수자 이체확인증 등록, 입금완료 메시지 |
| 매수자 | STEP1 | 거래 의사 메시지/PIN을 완료한 뒤 수령지갑 USDT 입금 자동검증 후 원화 계좌를 확인하고 이체확인증을 등록하는 화면 | 매칭준비중, 양측 PIN 완료, USDT 및 수수료 자동검증 완료 |
| 매수자 | STEP2 | 매도자 입금확인과 거래소 이체확인증 검토 완료 후 니모닉을 조회하는 화면 | 매도자 원화 확인 완료, 거래소 검토 완료 |

개선 시퀀스의 핵심 차단 조건은 다음과 같다.

1. USDT 자동검증 완료 전에는 매수자에게 원화 입금계좌를 노출하지 않는다.
2. 양측 PIN 완료 전에는 매칭취소가 가능하며, 지갑주소를 생성하거나 매도자에게 공개하지 않는다.
3. 양측 PIN 완료와 필요 시 ARS 동의 후 매칭확정으로 전환되면 임의 취소를 제한한다.
4. 이체확인증 등록과 본인 이체 확인 동의 전에는 매도자에게 입금완료 메시지를 송출하지 않는다.
5. 매도자 원화 입금확인 완료와 거래소 이체확인증 검토 완료가 모두 충족되기 전에는 니모닉 조회 버튼을 활성화하지 않는다.
6. 매수자가 니모닉을 조회하면 거래완료로 전환하고 접근 로그를 저장한다.
...
| 영역 | 구현 방식 |
|---|---|
| 전체 시퀀스 | 기존/개선 시퀀스 탭형 Mermaid Viewer |
...
서비스 이용약관과 별개로 매칭확정 전 고객이 확인하는 `이용약관 동의` 팝업을 둔다. 해당 팝업은 포블이 회원 간 USDT 안전거래 매칭을 중계한다는 점, 원화 이체와 USDT 전송은 회원 간 직접 이루어진다는 점, 원화 입금 여부는 이체확인증과 운영자 검토로 확인한다는 점, 매수자 수령지갑은 매수자 소유라는 점, 고객 귀책 없는 반환은 반환 수수료 없이 전액 반환을 원칙으로 한다는 점을 고지한다. 서비스 이용약관 게시글은 20개 조항 기준으로 구성하며 목적, 용어의 정의, 서비스 성격, 본인명의 원칙, 거래 신청과 매칭, 매칭확정/취소 제한, PIN 관리, 수령지갑 소유권, USDT 전송, 원화 이체, 매도자 입금확인, 니모닉 열람, 수수료, 보류/분쟁/반환, 금지행위, 증빙/로그/개인정보 처리, 서비스 제한, 책임 제한, 약관 변경, 기타를 포함한다.
...
운영 기준 게시글은 핵심고지, 서비스 제공 범위, 원화 이체대금 처리, 수수료 정책, 가상자산 이전 및 접근정보 제공, 거래확정 및 보류, USDT 반환 기준, 금지 행위를 포함한다. 이 문서는 고객 안내와 내부 처리 기준을 연결하는 문서로 사용하며, 각 꼭지는 3~4개 bullet 기준으로 짧게 정리한다.
...
Q&A는 고객센터 게시글 형태로 제공하며, 기존 초안의 조건부 암호화 지갑, 포블 키 결합, 고정 99%/101% 수수료 구조 등 현행 정책과 맞지 않는 표현은 사용하지 않는다. 고객이 실제로 궁금해하는 거래 흐름, 본인확인, 원화 입금, 수수료, 니모닉, 분쟁/반환 기준을 현행 정책 기준으로 안내한다.

| 질문 | 답변 |
|---|---|
| 이 서비스는 무엇인가요? | 포블 회원 간 USDT 매수/매도 의사를 매칭하고 거래 상태, 메시지, 지갑 생성, 입금 모니터링, 이체확인증, 분쟁 접수, 니모닉 열람 절차를 지원하는 중개 서비스입니다. |
| 포블이 거래 상대방인가요? | 아닙니다. 포블은 거래 진행 절차를 지원하며 원화 이체와 USDT 전송의 직접 당사자는 매수자와 매도자입니다. |
| 포블이 원화 거래대금을 보관하나요? | 보관하지 않습니다. 원화는 매수자가 매도자 본인명의 계좌로 직접 이체하며, 포블은 원화를 수취/보관/정산하지 않습니다. |
| 거래금액은 직접 입력할 수 있나요? | 직접 입력 방식이 아니라 포블이 제공하는 거래금액 구간 중 하나를 선택하는 방식으로 운영합니다. |
| 매칭되면 바로 거래가 확정되나요? | 바로 확정되지 않습니다. 매칭 준비 상태에서 거래 의사, 약관 동의, PIN 입력, 필요 시 ARS 또는 추가 확인을 거쳐 매칭확정 상태로 이동합니다. |
| 매칭확정 시 본인확인은 어떻게 하나요? | 약관 동의, PIN 입력, 계정 인증 상태, 필요 시 ARS 고객 동의 또는 추가 확인 절차로 거래 진행 의사를 확인할 수 있습니다. |
| ARS 확인은 항상 진행되나요? | 항상 진행되는 것은 아니며, 고액 거래, 위험 징후, 운영정책상 추가 확인이 필요한 경우 적용될 수 있습니다. |
| 매칭확정 후 취소할 수 있나요? | 매칭확정 후 단순 변심에 따른 임의 취소는 제한됩니다. 원화 입금 문제, USDT 수량 불일치, 증빙 불충분 등은 운영자 검토 또는 분쟁 상태로 처리합니다. |
| PIN은 왜 필요한가요? | PIN은 매칭확정, 분쟁 확인, 니모닉 열람 등 중요한 절차에서 본인 확인과 진행 의사 확인에 사용될 수 있습니다. |
| PIN을 잃어버리면 어떻게 되나요? | 거래 진행, 분쟁 확인, 니모닉 열람이 지연되거나 제한될 수 있으며 본인확인 및 운영정책에 따른 추가 확인이 필요할 수 있습니다. |
| 수령지갑은 누구 소유인가요? | 수령지갑은 매수자 소유 지갑이며 포블 소유 지갑이 아닙니다. 매수자는 거래완료 후 니모닉을 확인하고 외부 지갑 복구 또는 이전으로 자산 소유를 확인해야 합니다. |
| 니모닉은 언제 볼 수 있나요? | 매칭확정, USDT 입금, 수수료 입금, 이체확인증 등록, 매도자 입금확인, 필요 시 추가 확인 등 거래진행 조건이 충족된 후 열람할 수 있습니다. |
| 니모닉을 열람하면 어떤 의미인가요? | 매수자가 니모닉을 최초 열람하면 거래완료가 확정되며 수령지갑의 접근정보를 인수한 것으로 봅니다. |
| 니모닉은 언제까지 재열람할 수 있나요? | 최초 열람 시점부터 7일간만 재열람할 수 있습니다. 7일 후에는 재열람이 불가하므로 안전하게 백업하거나 외부 지갑 복구/이전을 완료해야 합니다. |
| 수수료는 원화로 내나요? | 아닙니다. 매수자는 화면에 표시된 원화 거래금액을 매도자에게 이체하며, 서비스 수수료와 프리미엄은 USDT 기준으로 산정되어 거래 화면 기준에 따라 반영됩니다. |
| USDT 기준가와 수수료율은 고정인가요? | 고정값으로 단정하지 않습니다. 기준가, 수수료율, 프리미엄율, 산정방식, 적용시점은 시장상황과 운영정책에 따라 변경될 수 있습니다. |
| 원화 입금 후 무엇을 해야 하나요? | 매수자는 원화 이체 후 이체확인증을 등록해야 하며, 거래금액, 송금일시, 수취인, 수취계좌, 금융기관, 거래번호 등 확인 정보가 포함되어야 합니다. |
| 이체확인증을 등록하면 원화 입금이 확정되나요? | 아닙니다. 이체확인증 확인은 거래진행 조건 확인 절차이며 실제 원화 입금 보증을 의미하지 않습니다. |
| 매도자는 원화 입금을 어떻게 확인하나요? | 매도자는 본인 명의 계좌에서 실제 원화 입금 여부, 금액, 입금자명, 입금시각, 거래조건 일치 여부를 직접 확인한 뒤 입금확인 버튼을 클릭해야 합니다. |
| 제3자 명의 입금은 가능한가요? | 허용되지 않습니다. 제3자 명의 입금, 입금자명 불일치, 차명거래가 의심되면 보류 또는 분쟁검토 상태로 전환될 수 있습니다. |
| 어떤 경우 분쟁검토로 전환되나요? | 원화 미입금, 매도자 입금 미확인, 이체확인증 불충분, 위조 의심, 제3자 입금, USDT 수량 불일치, 수수료 미입금, 지갑주소 오류, 니모닉 열람 관련 이의가 있으면 전환될 수 있습니다. |
| USDT 반환 요청은 언제 검토되나요? | 원화 입금 미확인, 이체확인증 불충분, 분쟁 상태에서 매도자의 반환 요청이 있는 경우 거래상태, 니모닉 열람 여부, 전송상태, 귀책사유, 증빙자료를 기준으로 검토합니다. |
| 고객 귀책이 없는데 반환 수수료가 있나요? | 고객 귀책이 없는 반환은 반환 수수료 없이 입금 USDT 전액 반환을 원칙으로 합니다. 다만 불가피한 네트워크 비용이나 외부 비용은 운영정책에 따를 수 있습니다. |
| 니모닉 열람 후에도 반환이 가능한가요? | 니모닉 열람 후에는 거래완료가 확정되므로 원칙적으로 반환이 제한될 수 있습니다. 실제 처리는 거래 상태, 증빙, 귀책 여부에 따라 운영/준법 검토 기준으로 판단합니다. |
| 고객센터 문의 시 어떤 자료가 필요한가요? | 거래번호, 이체확인증, TXID, 지갑주소, 입금자명, 입금시각, 계좌 확인 자료, 오류 화면 캡처 등을 제출하면 검토가 빠르게 진행됩니다. |
